...
public class DBManager {
    ...
    private final String passwordServer;
    private final String usernameServer;
    ...
    private static final String GET_CLIENT_BY_ID_REQUEST = "SELECT * FROM clients WHERE id = ?";
    ...
    public void insertNewClient(ClientIdentificate client)
            throws SQLException {
    ...
    public void updateFlatById(Flat updating, ClientIdentificate client, Integer idSwap)
        ...
        if (result.next()) {
            PreparedStatement getLoginReq = connection.prepareStatement(GET_CLIENT_BY_ID_REQUEST);
            getLoginReq.setInt(1, result.getInt(11));
            ResultSet resultSetOfClient = getLoginReq.executeQuery();
            resultSetOfClient.next();
            if (!(client.getLogin().equals(resultSetOfClient.getString(2))))
        ...
        try {
            ...
            connection.setAutoCommit(false);
            System.out.println("id coord has been inserting");
            int idCoord = insertNewCoordinates(updating.getCoordinates());
            System.out.println("id house has been inserting");
            int idHouse = insertNewHouse(updating.getHouse());
            System.out.println("id furnish gas been inserting");
            ...
            if (preparedStatement.execute()) {
                System.out.println("End of command UPDATE, successfully");
            } else {
